...
public class DocUtils {
    ...
    public static void SampleStringField(Object bean, Field field) {
        ...
        if (comment != null) {
            BeanUtil.setProperty(bean, field.getName(), comment.value());
        } else {
            BeanUtil.setProperty(bean, field.getName(), "xxxxxx");
        }
    }
        ...
        for (Field field : fields) {
            ...
            if (IsBasicType(field.getType())) {
                ...
                if (field.getName().endsWith("_amt") || field.getName().endsWith("_amount")) {
                    BeanUtil.setPropertyForcedSilent(bean, field.getName(), 100);
                    continue;
                }

                if (field.getName().equals("page_num")) {
                    BeanUtil.setPropertyForcedSilent(bean, field.getName(), 1);
                    continue;
                }

                if (field.getName().equals("page_size")) {
                    BeanUtil.setPropertyForcedSilent(bean, field.getName(), 10);
                    continue;
                }
                ...
                if (BeanUtil.getPropertySilently(bean, field.getName()) != null) {
                    continue;
                }
                ...
                if (field.getType().getTypeName().equals(String.class.getTypeName())) {
                    Comment comment = field.getAnnotation(Comment.class);
                    if (comment != null) {
                        BeanUtil.setPropertyForcedSilent(bean, field.getName(), comment.value());
                    } else {
                        BeanUtil.setPropertyForcedSilent(bean, field.getName(), "XXXXXX");
                    }
                    continue;
                }

                if (field.getType().getTypeName().equals(Date.class.getTypeName())) {
                    BeanUtil.setPropertyForcedSilent(bean, field.getName(), new Date());
                    continue;
                }
                ...
                if (field.getType().getTypeName().equals(boolean.class.getTypeName()) ||
                    ...
                    BeanUtil.setPropertyForcedSilent(bean, field.getName(), true);
                    continue;
                }
            } else {
                ...
                if (field_type instanceof ParameterizedType) {
                    ...
                    Object field_obj = BeanUtil.getPropertySilently(bean, field.getName());
                    ...
                    if (field_obj == null) {
                        ...
                        BeanUtil.setPropertyForcedSilent(bean, field.getName(), field_obj);
