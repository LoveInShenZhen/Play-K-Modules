...
public class UploadForm {
    ...
    private Http.MultipartFormData.FilePart<File> GetFilePart() {
        Http.MultipartFormData<File> body = Controller.request().body().asMultipartFormData();
        Http.MultipartFormData.FilePart<File> filePart = body.getFile(UploadForm.file_field_name);
        return filePart;
    }

    private String WarteRmarkPicPath() {
        return Hub.configuration().getString("K.WarteRmarkPicPath", "/conf/resource/wartermark.png");
    }

    private String WaterPath() {
        return Hub.application().getFile(WarteRmarkPicPath()).getAbsolutePath();
    }

    public long SaveAttachment() throws IOException {
        DynamicForm post_data = Hub.formFactory().form().bindFromRequest();
        ...
        Http.MultipartFormData.FilePart<File> filePart = GetFilePart();
    ...
    public static String GetUploadDir() throws IOException {
        String upload_dir = "";
        String cfg = Hub.configuration().getString("file_upload_dir", "");
