...
public class AutoRetryTask implements Runnable {
    ...
    public void setWrapperTask(Runnable task) {
        ...
        taskJsonData = Helper.INSTANCE.ToJsonString(task);
    }
    ...
    public void run() {
        ...
        try {
            task = (Runnable) Helper.INSTANCE.FromJsonString(taskJsonData, Class.forName(taskClassName));
        ...
        if (!result.OK) {
            ...
            Logger.warn("Run a {} failed. {}\n{}",
                    taskClassName,
                    Helper.INSTANCE.ToJsonStringPretty(this),
                    Helper.INSTANCE.StackTraceOfEx(result.ex));
            if (ContinueRetry()) {
                addRetryTask(task);
            }
        }
    ...
    public static void AddAutoRetryTask(Runnable task, boolean requireSeq, String seqType,
        ...
        Date planRunTime = Helper.INSTANCE.IncTimeInSec(new Date(), delayTimeForRetry);
        PlanTask.Companion.addTask(retryTask, seqType, requireSeq, planRunTime, tag);
    }
