...
class JsonApiAction : Action<JsonApi>() {
    ...
    private var before_call: Date? = null
    ...
    private fun log(ex_msg: String) {
        ...
        if (record_api) {
            ...
            val request = JsonpController.request()
            val now = Date()
            ...
            val ins_sql = String.format("insert api_log (`guid`, `log_time`, `api_method`, `api_path`, `api_url`, "
                    ...
                    log_guid.toString(),
                    Helper.Date2Str(Date(), Helper.DB_DATE_FMT),
                    request.method(),
                    request.path(),
                    request.uri(),
                    Helper.ToJsonStringPretty(request.body().asJson()),
                    java.lang.Long.toString(now.time - this.before_call!!.time),
                    this.hostName,
